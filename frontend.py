...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<div class='sidebar-title'>🌍 Trivedi AI Travel Planner</div>", unsafe_allow_html=True)
    st.markdown("---")

    thread_id = st.text_input("👤 User ID", value="Akhilesh_Trivedi",
...
<div class="hero-wrapper">
    ...
    <div class="hero-content">
        ...
        <div class="hero-sub">Four specialized agents work together — finding flight guidance, hotel options, weather insights, and building your complete itinerary.</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
QUICK = [
    "7-day Japan trip from Delhi under ₹2 lakh",
    "5-day Paris trip from Delhi",
    "Dubai weekend trip from Delhi",
    "10-day Bali backpacking trip",
]
qcols = st.columns(len(QUICK))
for index, (qc, label) in enumerate(zip(qcols, QUICK)):
    with qc:
        if st.button(label, key=f"quick_{index}"):
            st.session_state.travel_query = label

user_query = st.text_area(
    "Travel request",
    key="travel_query",
    placeholder=(
        "Example: Plan a complete 6-day Vietnam trip from Delhi for two adults, "
        "including flights, hotels, weather and a day-by-day itinerary under ₹1.5 lakh"
    ),
    height=110,
...
def render_text(value, empty_message):
    """Render strings and structured MCP results safely in Streamlit."""
...
if generate:
    ...
    clean_thread_id = thread_id.strip() or "default_travel_user"
    ...
    if not clean_query:
        ...
        # main.py expects every TravelState field to exist before graph execution.
        initial_state = {
        ...
        try:
            with st.spinner("The agents are preparing your travel plan..."):
                    ...
                    for node_name, state_update in chunk.items():
                        ...
                        with st.status(
                            ...
                            if node_name == "flight_agent":
                                ...
                                render_text(text, "No flight information was returned.")

                            elif node_name == "hotel_agent":
                                text = state_update.get("hotel_results", "")
                                collected["hotel_results"] = text
                                render_text(text, "No hotel information was returned.")

                            elif node_name == "weather_agent":
                                text = state_update.get("weather_results", "")
                                collected["weather_results"] = text
                                render_text(text, "No weather information was returned.")

                            elif node_name == "itinerary_agent":
                                text = state_update.get("itinerary", "")
                                collected["itinerary"] = text
                                render_text(text, "No itinerary was generated.")
            ...
            st.error("The travel plan could not be generated.")
            st.exception(exc)
            st.info(
                "Check the terminal for the complete traceback. Also verify DATABASE_URL, "
                "Groq/Tavily credentials, MCP services, and PostgreSQL connectivity."
            )
        else:
            st.markdown(
                f"""
                <div class="metric-row">
                    <div class="metric-box"><div class="metric-val">{completed_agents}</div><div class="metric-lbl">Agents Run</div></div>
            ...
            # main.py has no final_agent. The itinerary is the final generated plan.
